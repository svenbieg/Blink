...
VOID __gxx_personality_v0(UINT flags, UnwindException* exc, UnwindContext* context)
{
...
SIZE_T instr_offset=context->ProgramCounter-frame_start;
while(lsda.GetPosition()<callsite_end)
	...
	while(action_id)
		...
		if(type_id>0)
			...
			SIZE_T rodata_start=(SIZE_T)&__rodata_start;
			SIZE_T rodata_end=(SIZE_T)&__rodata_end;
			if(catch_id>=rodata_start&&catch_id<rodata_end)
