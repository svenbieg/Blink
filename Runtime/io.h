...
__always_inline void io_clear(rw32_t& reg, uint32_t mask) { reg&=~mask; }
__always_inline uint8_t io_read(ro8_t& reg) { return reg; }
__always_inline uint16_t io_read(ro16_t& reg) { return reg; }
...
__always_inline uint64_t io_read(ro64_t& reg) { return reg; }
__always_inline uint64_t io_read(ro64_t& reg, uint64_t mask) { return reg&mask; }
__always_inline uint64_t io_read(ro64_t& reg, bits64_t const& bits) { return (reg>>bits.shift)&bits.mask; }
__always_inline void io_set(rw32_t& reg, uint32_t mask) { reg|=mask; }
__always_inline void io_write(rw8_t& reg, uint8_t value) { reg=value; }
__always_inline void io_write(rw16_t& reg, uint16_t value) { reg=value; }
...
__always_inline void io_write(rw16_t& reg, bits16_t const& bits, uint16_t value) { uint16_t u=reg; u&=~(bits.mask<<bits.shift); u|=(value<<bits.shift); reg=u; }
...
__always_inline void io_write(rw64_t& reg, uint64_t value) { reg=value; }
