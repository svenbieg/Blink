...
if(handler)
	...
	BitHelper::Set(m_IrqMask, pin_mask);
	m_Handlers[id]=handler;
	m_Parameters[id]=param;
	if(irq_mask==0)
		m_PcieHost->SetInterruptHandler(Rp1Irq::IoBank0, this, &GpioHost::HandleInterrupt);
	UINT flags=0;
	switch(mode)
		{
		case GpioIrqMode::Edge:
			{
			BitHelper::Set(flags, CTRL_IRQMASK_EDGE_HIGH);
			BitHelper::Set(flags, CTRL_IRQMASK_EDGE_LOW);
			break;
			}
	...
	BitHelper::Clear(m_IrqMask, pin_mask);
