...
const UINT GICD_CTRL_DISABLE			=0;
const UINT GICD_CTRL_ENABLE_GROUP0		=(1<<0);
const UINT GICD_CTRL_ENABLE_GROUP1		=(1<<1);
const UINT GICD_PRIORITY_DEFAULT		=0xA0A0A0A0;
const UINT GICD_TARGET_CORE0			=0x01010101;
const UINT GICD_CONFIG_LEVEL_TRIGGERED	=0;
const BITS GICD_SGIR_CPU_TARGET_LIST	={ 0xF, 16 };
...
const UINT GICC_CTRL_ENABLE		=1;
const UINT GICC_PMR_PRIORITY	=0xF0;
const UINT GICC_IAR_IRQ_MASK	=0x3FF;
...
auto handler=s_IrqHandlers[irq];
if(s_TaskMonitor)
...
InitializeSecondary();
Route(Irq::TaskSwitch, IrqTarget::All);
SetHandler(Irq::TaskSwitch, HandleTaskSwitch);
}
...
InterruptHandler* Interrupts::s_IrqHandlers[IRQ_COUNT]={ nullptr };
