...
if(handler)
	...
	if(BitHelper::Get(irq_mask, RP1_IRQ_EDGE_MASK))
		...
		{
		IoHelper::Write(intc->MSIX_CFG_SET[irq], MSIX_CFG_IACK_EN);
		}
	IoHelper::Write(intc->MSIX_CFG_SET[irq], MSIX_CFG_EN);
	}
else
	{
	IoHelper::Write(intc->MSIX_CFG_CLEAR[irq], MSIX_CFG_EN);
	IoHelper::Write(intc->MSIX_CFG_CLEAR[irq], MSIX_CFG_IACK_EN);
...
{
UINT bits=BitHelper::BitCount(size);
if(bits<12)
	return 0;
if(bits<16)
